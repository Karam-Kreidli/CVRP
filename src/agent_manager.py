"""
Stage 2 - Fleet Manager: Actor-Critic agent for adaptive solver strategy.

Consumes the global graph embedding from Stage 1 (GNNEncoder) concatenated
with a 4-dim solver statistics vector to select solver parameter regimes.
...
to learn when and how hard to push for fleet reduction vs route optimization.
...
Action space (discrete, 6):
  0 = POLISH            — Default params, optimize routes within current fleet
  1 = MILD_PRESSURE     — Light penalty (increase=2.0) for gentle fleet reduction
  2 = MODERATE_PRESSURE — Medium penalty (increase=5.0) for steady fleet reduction
  3 = AGGRESSIVE_PRESSURE — Heavy penalty (increase=10.0) for forcing route merges
  4 = EXPLORE_NEW_SEED  — Fresh random seed with default params
  5 = EXPLORE_PRESSURE  — Fresh random seed + moderate penalty (escape + reduce)
...
class FleetManager(nn.Module):
    """Actor-Critic network for solver strategy selection.
    ...
    Args:
        ...
        num_actions: Discrete action space size (default: 6).
    """
    ...
    def __init__(
        ...
        # Actor head — policy logits over strategies
    ...
    def forward(
        ...
        Returns:
            action_logits: (B, 6) raw logits over strategies (masked set to -1e4).
